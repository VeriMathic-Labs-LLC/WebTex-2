...
async function processMathExpressions(expressions) {
  ...
  for (const expr of expressions) {
    ...
    mathElement.className = `webtex-math ${
      expr.display ? "webtex-display" : "webtex-inline"
    }`;

    const result = await renderMathExpression(
      expr.tex,
      expr.display,
      mathElement
    );
...
(async function main() {
  ...
  isEnabled =
    ...
    isDomainAllowed ||
    isTestFile ||
    location.pathname.includes("test-simple.html");
  ...
  chrome.runtime.onMessage.addListener(async (msg, _sender, sendResponse) => {
    if (msg.action === "domain-updated" && msg.allowed) {
      const newIsEnabled =
        ...
        msg.allowed.includes(location.hostname);
...
async function enableRendering() {
  ...
  observer = new MutationObserver(
    debounce((muts) => {
      if (
        mutationsOnlyRipple(muts) ||
        userIsSelectingText() ||
        typingInsideActiveElement(muts)
      ) {
        return;
      }
...
function nodeIsEditable(n) {
  ...
  return (
    n.isContentEditable ||
    (n.nodeType === 1 && /^(INPUT|TEXTAREA|SELECT)$/.test(n.tagName))
  );
}
