...
class LatexRenderer {
  ...
  processComplexExpressions(html) {
    try {
      ...
      html = html.replace(/\\text\{_([^}]+)\^([^}]+)\s+([^}]+)\}/g, '<span style="font-style: normal;"><sub>$1</sub><sup>$2</sup>$3</span>');
      
      // Pattern: \text{Z^A N} -> <span>Z<sup>A</sup>N</span>
      html = html.replace(/\\text\{([A-Za-z]+)\^([^}]+)\s+([^}]+)\}/g, '<span style="font-style: normal;">$1<sup>$2</sup>$3</span>');
      
      // Pattern: \text{_Z^A N} -> <span><sub>Z</sub><sup>A</sup>N</span>
      html = html.replace(/\\text\{_([^}]+)\^([^}]+)\s+([^}]+)\}/g, '<span style="font-style: normal;"><sub>$1</sub><sup>$2</sup>$3</span>');
      
      // Pattern: \text{{Z-2}^{A-4} N'} -> <span><sub>Z-2</sub><sup>A-4</sup>N'</span>
      html = html.replace(/\\text\{\{([^}]+)\}\^\{([^}]+)\}\s+([^}]+)\}/g, '<span style="font-style: normal;"><sub>$1</sub><sup>$2</sup>$3</span>');
