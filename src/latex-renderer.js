...
class LatexRenderer {
  ...
  processComplexExpressions(html) {
    try {
      ...
      html = html.replace(
        /\\text\{e\^-\}/g,
        '<span style="font-style: normal;">e⁻</span>'
      );
      html = html.replace(
        /\\text\{e\^\+\}/g,
        '<span style="font-style: normal;">e⁺</span>'
      ...
      html = html.replace(
        /\\text\{neutrino\}/g,
        '<span style="font-style: normal;">ν</span>'
      );
      html = html.replace(
        /\\text\{He\}/g,
        '<span style="font-style: normal;">He</span>'
      );
      html = html.replace(
        /\\text\{Rn\}/g,
        '<span style="font-style: normal;">Rn</span>'
      );
      html = html.replace(
        /\\text\{Ra\}/g,
        '<span style="font-style: normal;">Ra</span>'
      );
      html = html.replace(
        /\\text\{C\}/g,
      ...
      html = html.replace(
        /\\text\{N\}/g,
        '<span style="font-style: normal;">N</span>'
      );
      html = html.replace(
        /\\text\{Ne\}/g,
        '<span style="font-style: normal;">Ne</span>'
      );
      html = html.replace(
        /\\text\{F\}/g,
        '<span style="font-style: normal;">F</span>'
      );

      // Nuclear notation: ^{A}_{Z}X (handle various formats)
      html = html.replace(
        /\^\{([^}]+)\}_\{([^}]+)\}([A-Za-z])/g,
        "<sup>$1</sup><sub>$2</sub>$3"
      );
      html = html.replace(
        /\^\{([^}]+)\}_\{([^}]+)\}\\text\{([^}]+)\}/g,
  ...
  processTextNode(textNode) {
    ...
    if (
      ...
      parent.closest(
        "script, style, textarea, pre, code, input, select, button"
      )
    ) {
      return;
    }
